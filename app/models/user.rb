class User < ActiveRecord::Base
  ...
  has_many :courses, :through => :course_subscription
  ...
  def following_courses
  	self.courses.where(:course_subscriptions => {:accepted => 1})
  end

  def pending_courses
  	self.courses.where(:course_subscriptions => {:accepted => 0})
  end
  ...
  def follow(course)
 		course.ask_to_subscribe self
 		course.accept_subscription self
  end
